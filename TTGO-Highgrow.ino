...
const String device_name = "HighGrow";
const String device_placement = "Arbetsrum";
...
// By doing this you will get the same readout for each unit. Replace the value below for the dry condition, and the 100% humid condition, and you are done.
{
  ...
  Serial.println(soil);
  return map(soil, 1920, 3197, 100, 0); // Soil defaults - change them to your calibration data
}
...
void saveConfiguration(const Config & config) {
  ...
  JsonObject devicename = doc.createNestedObject("devicename");
  devicename["devicename"] = device_name;
  JsonObject deviceid = doc.createNestedObject("deviceid");
  ...
  humidity["humidity"] = config.humid;
